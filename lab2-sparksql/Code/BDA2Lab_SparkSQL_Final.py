...
filtered = temperature.select("stationNum","date", F.year(F.col('date')).alias("year"),                                     F.month(F.col("date")).alias("month"),F.col("airTemp").cast("float"))
...
filtered = filtered.groupBy('date', 'stationNum').agg(F.min('airTemp').alias('minTemp'),                                                     F.max('airTemp').alias('maxTemp'))

tempavg = filtered.select('date','stationNum',((F.col('minTemp')+F.col('maxTemp'))/2.0).alias('dailyAvg'))                    .groupBy(F.year(F.col('date')),F.month(F.col('date')),'stationNum')                    .agg(F.avg('dailyAvg').alias('avgMonthlyTemperature'))            .orderBy('avgMonthlyTemperature',ascending=False)
